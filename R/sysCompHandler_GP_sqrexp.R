...
createSysCompGPHandler <- function() {
  ...
  map <- function(expDt, sysDt, gpDt, mapAssignment, ret.mat=TRUE) {
    
    setkey(gpDt, EXPID)
    stopifnot(is.data.table(sysDt),
              is.data.table(gpDt))
    stopifnot("IDX" %in% names(sysDt))
    gpExpDt <- expDt[REAC %in% mapAssignment[,REAC]]
    
    sysDtGpObs <- sysDt[(GPTYPE == "sqrexp" & ERRTYPE == "pw")][,IDX := seq(.N)]
    resDt <- sysDtGpObs[, {

      curExpDt <-  gpExpDt[REAC %in% mapAssignment[EXPID==.BY, REAC]]
      curGPSpec <- gpDt[J(.BY[["EXPID"]])]
      curSigma <- curGPSpec[PARNAME == "sigma", PARVAL]
      curLen <- curGPSpec[PARNAME == "len", PARVAL]

      covMat <- curSigma^2 * exp(-0.5/curLen^2 * (outer(curExpDt$L1, EN, `-`))^2)

      
      packedCov <- list(IDX1 = rep(curExpDt$IDX, length(.SD$IDX)), IDX2 = rep(.SD$IDX, each=length(curExpDt$IDX)), X = as.vector(covMat))

      packedCov
    }, by="EXPID"]
    
    resDt[,EXPID := NULL]
    resDims <- c(nrow(expDt), nrow(sysDtGpObs))

    returnSparseMatrix(resDt, resDims, ret.mat)
  }
  
  covExact <- function(expDt, gpDt, mapAssignment, ret.mat=TRUE) {
    
    setkey(gpDt, EXPID)
    stopifnot(is.data.table(expDt),
              is.data.table(gpDt))
    stopifnot("IDX" %in% names(expDt))
    gpExpDt <- expDt[REAC %in% mapAssignment[,REAC]]
    
    resDt <- gpExpDt[, {
      curGPSpec <-  gpDt[EXPID %in% mapAssignment[REAC==.BY, EXPID]]
      curSigma <- curGPSpec[PARNAME == "sigma", PARVAL]
      curLen <- curGPSpec[PARNAME == "len", PARVAL]
      curNugget <- curGPSpec[PARNAME == "nugget", PARVAL]
      
      covMat <- curSigma^2 * exp(-0.5/curLen^2 * (outer(L1, L1, `-`))^2)
        diag(covMat) <- diag(covMat) + curNugget^2
    ...
    #resDt[,EXPID := NULL]
    resDims <- c(nrow(expDt), nrow(expDt))
    returnSparseMatrix(resDt, resDims, ret.mat)
  }

  invCovExact <- function(expDt, gpDt, mapAssignment, ret.mat=TRUE) {
    ...
    resDt <- gpExpDt[, {
      ...
      covMat <- curSigma^2 * exp(-0.5/curLen^2 * (outer(L1, L1, `-`))^2)
        diag(covMat) <- diag(covMat) + curNugget^2 + UNC^2
      invCovMat <- solve(covMat)
      list(IDX1 = rep(IDX, length(IDX)), IDX2 = rep(IDX, each=length(IDX)), X = as.vector(invCovMat))
    }, by="REAC"]
  ...
  mapExact <- function(expDt, gpDt, mapAssignment, ret.mat=TRUE) {
